...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        sum_gamma_rewards = np.sum(rewards * np.power(self.gamma, t))
        list_of_discounted_returns = np.ones(len(rewards)) * sum_gamma_rewards
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for start_t in range(len(rewards)):
            t = np.arange(start_t, len(rewards))
            sum_gamma_rewards = np.sum(rewards[t] * np.power(self.gamma, t - start_t))
            list_of_discounted_cumsums.append(sum_gamma_rewards)
