...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        """
        list_of_discounted_returns = np.ones(len(rewards)) * np.sum(rewards * np.power(self.gamma, np.arange(len(rewards))))
    ...
    def _discounted_cumsum(self, rewards):
        ...
        """
        list_of_discounted_cumsums = np.array([np.sum(rewards[np.arange(t, len(rewards))] * np.power(self.gamma, np.arange(t, len(rewards)) - t)) for t in range(len(rewards))])
