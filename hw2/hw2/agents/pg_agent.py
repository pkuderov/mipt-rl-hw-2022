...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs, rews_list, q_values, terminals):
        ...
        if self.nn_baseline:
            ...
            values = values_unnormalized * np.std(q_values) + np.mean(q_values) # TODO
        ...
        if self.standardize_advantages:
            ...
            advantages = normalize(advantages, np.mean(advantages), np.std(advantages)) # TODO
    ...
    def _discounted_return(self, rewards):
        ...
        T = len(rewards)
        gammas = np.empty(T)
        gammas.fill(self.gamma)
        gammas[0] = 1
        gammas = gammas.cumprod()

        list_of_discounted_returns = [np.sum(gammas * rewards)] * T
    ...
    def _discounted_cumsum(self, rewards):
        ...
        discounted_returns = np.cumsum(np.flip(gammas * rewards))
        discounted_returns = np.flip(discounted_returns) / gammas
        list_of_discounted_cumsums = list(discounted_returns)
