...
class MLPPolicySL(MLPPolicy):
    ...
    def update(
        ...
        pred_act = self(torch.Tensor(observations).to(ptu.device)) #??
        # pred_act = self.get_action(observations)
        act = torch.Tensor(actions).to(ptu.device)

        loss = self.loss(pred_act, act)
        loss.backward()
        self.optimizer.step()
